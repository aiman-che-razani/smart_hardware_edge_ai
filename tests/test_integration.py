...
def test_corruption_creates_gaps_not_false_continuous_windows(tmp_path):
    result=run(tmp_path,3,"NORMAL",corrupt_every=300,drop_every=211)
    assert result["parser_errors"]>0
...
def test_group_split_and_model_workflow(tmp_path):
    data=tmp_path/'data'
    for condition in ("NORMAL","IMBALANCE_HIGH"):
        for seed in range(3):
            run(data,2,condition,seed=seed+10)
